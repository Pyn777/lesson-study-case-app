...
export async function ensureSchema() {
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS lesson_study_urinalysis (
      id BIGSERIAL PRIMARY KEY,
      ...
      case_id TEXT,
      case_title TEXT,
      expected_organism TEXT,
      final_identification TEXT,
      gram_reaction TEXT,
      rapid_test TEXT,
      followup_test TEXT,
      score INTEGER,
      score_out_of INTEGER,
      submitted_at TIMESTAMPTZ,
      received_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    )
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS idx_lesson_study_urinalysis_study
    ON lesson_study_urinalysis (study_id, semester)
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS lesson_study_roleplay (
      id BIGSERIAL PRIMARY KEY,
      roleplay_id TEXT UNIQUE NOT NULL,
      session_id TEXT,
      study_id TEXT,
      semester TEXT,
      course TEXT,
  ...
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS idx_lesson_study_roleplay_study_module
    ON lesson_study_roleplay (study_id, module)
  `;
  await sql`
...
export async function insertRolePlay(entry) {
  const sql = await ensureSchema();
  const roleplayId = String(entry?.roleplayId || "").trim();
  if (!roleplayId) throw new Error("roleplayId is required");

  const [existing] = await sql`
    SELECT id
    FROM lesson_study_roleplay
    WHERE roleplay_id = ${roleplayId}
  `;
  if (existing) return { duplicate: true, id: existing.id };

  const [row] = await sql`
    INSERT INTO lesson_study_roleplay (
      roleplay_id, session_id, study_id, semester, course, section,
      module, role_name, claim_text, evidence_text, action_text, submitted_at
    )
    VALUES (
      ${roleplayId},
      ...
      ${String(entry.roleName || "")},
      ${String(entry.claimText || "")},
      ${String(entry.evidenceText || "")},
      ${String(entry.actionText || "")},
...
export async function getRolePlayEntries() {
  const sql = await ensureSchema();
  return sql`
    SELECT
      id,
      roleplay_id AS "roleplayId",
      session_id AS "sessionId",
      study_id AS "studyId",
      semester,
      course,
...
export async function insertUrinalysisIntegration(entry) {
  const sql = await ensureSchema();
  ...
  const [existing] = await sql`
    SELECT id
    FROM lesson_study_urinalysis
    WHERE integration_id = ${integrationId}
  `;
  if (existing) return { duplicate: true, id: existing.id };

  const [row] = await sql`
    INSERT INTO lesson_study_urinalysis (
      integration_id, session_id, study_id, semester, course, section,
      case_id, case_title, expected_organism, final_identification,
      gram_reaction, rapid_test, followup_test, score, score_out_of, submitted_at
    )
    VALUES (
      ${integrationId},
      ${entry.sessionId || null},
      ...
      ${entry.section || null},
      ${entry.caseId || null},
      ${entry.caseTitle || null},
      ${entry.organism || null},
      ${entry.finalIdentification || null},
      ${entry.gramReaction || null},
      ${entry.rapidTest || null},
      ${entry.followupTest || null},
      ${Number.isFinite(Number(entry.score)) ? Number(entry.score) : null},
      ${Number.isFinite(Number(entry.outOf)) ? Number(entry.outOf) : null},
      ${entry.submittedAt ? new Date(entry.submittedAt) : new Date()}
    )
    RETURNING id
  `;
...
export async function getUrinalysisIntegrations() {
  const sql = await ensureSchema();
  return sql`
    SELECT
      id,
      ...
      expected_organism AS "organism",
      final_identification AS "finalIdentification",
      gram_reaction AS "gramReaction",
      rapid_test AS "rapidTest",
      followup_test AS "followupTest",
      score,
      score_out_of AS "outOf",
      submitted_at AS "submittedAt",
      received_at AS "receivedAt"
    FROM lesson_study_urinalysis
    ORDER BY received_at DESC, id DESC
